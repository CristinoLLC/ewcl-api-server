...
export default function BenchmarkStats({ benchmarks }: BenchmarkStatsProps) {
  ...
  const [filteredBenchmarks, setFilteredBenchmarks] = useState(benchmarks)
  ...
  const filterBenchmarks = () => {
    ...
    if (riskLevel !== 'All') {
      filtered = filtered.filter(b => b.riskLevel === riskLevel);
    }
  ...
  const exportToCSV = () => {
    const csvRows = [
      ...
      ...filteredBenchmarks.map(b => [
        ...
        b.collapseScore,
        b.riskLevel,
        b.timestamp instanceof Timestamp ? b.timestamp.toDate().toISOString() : b.timestamp,
      ]),
    ];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex space-x-4 mb-4">
        ...
        <button onClick={filterBenchmarks} className="bg-blue-500 text-white rounded p-2">Filter</button>
        <button onClick={exportToCSV} className="bg-green-500 text-white rounded p-2">Export to CSV</button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredBenchmarks.map(benchmark => (
          <div key={benchmark.id} className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
            ...
            <p>Timestamp: {benchmark.timestamp instanceof Timestamp ? benchmark.timestamp.toDate().toLocaleString() : benchmark.timestamp}</p>
            <button onClick={() => openModal(benchmark)} className="mt-2 bg-blue-500 text-white rounded p-2">View 3D</button>
          </div>
        ))}
      </div>
      ...
      <Modal isOpen={modalIsOpen} onRequestClose={closeModal} contentLabel="3D Viewer">
        <h2 className="text-xl font-semibold mb-4">3D Structure Viewer</h2>
        {selectedBenchmark && (
