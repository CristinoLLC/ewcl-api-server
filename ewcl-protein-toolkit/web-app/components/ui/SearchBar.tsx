  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredBenchmarks.map(benchmark => (
          <div key={benchmark.id} className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
            ...
            <p>Submitted on: {benchmark.timestamp instanceof Timestamp ? benchmark.timestamp.toDate().toLocaleDateString() : new Date(benchmark.timestamp).toLocaleDateString()}</p>
            <button onClick={() => openModal(benchmark)} className="mt-2 bg-blue-500 text-white rounded p-2">
              View 3D
            </button>
          </div>
        ))}
      ...
      <Modal isOpen={modalIsOpen} onRequestClose={closeModal} contentLabel="3D Viewer">
        <h2 className="text-xl font-semibold mb-4">3D Viewer for {selectedBenchmark?.proteinName}</h2>
        {selectedBenchmark && (
      ...
      {/* (The rest of your existing BenchmarkStats component code) */}
    </div>
  )
}