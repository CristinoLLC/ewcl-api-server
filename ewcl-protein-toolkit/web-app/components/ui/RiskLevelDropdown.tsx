...
export default function BenchmarkStats({ benchmarks }: BenchmarkStatsProps) {
  ...
  const filterBenchmarks = () => {
    const filtered = benchmarks.filter(b => {
      ...
      const matchesRiskLevel = riskLevel === 'All' || b.riskLevel === riskLevel
  ...
  const exportToCSV = () => {
    const csvRows = [
      ['ID', 'Protein Name', 'Avg Score', 'Collapse Score', 'Max Entropy Residue', 'Risk Level', 'Timestamp'],
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredBenchmarks.map(benchmark => (
          <div key={benchmark.id} className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
            ...
            <button onClick={() => handleView3D(benchmark)} className="mt-2 bg-indigo-600 text-white rounded p-2">
              View 3D
            </button>
          </div>
        ))}
      ...
      {showModal && selectedBenchmark && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-4 rounded-lg">
            <h4 className="text-lg font-semibold mb-2">3D View of {selectedBenchmark.proteinName}</h4>
            <MolViewer pdbFile={selectedBenchmark.id} entropyMap={selectedBenchmark.maxEntropyResidue} />
            <button onClick={closeModal} className="mt-2 bg-red-500 text-white rounded p-2">
              Close
            </button>
          </div>
        </div>
