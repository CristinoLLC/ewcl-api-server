...
export default function BenchmarkStats({ benchmarks }: BenchmarkStatsProps) {
  ...
  const filterBenchmarks = () => {
    ...
    if (startDate) {
      ...
      filtered = filtered.filter(b => b.timestamp.toDate().getTime() >= start);
    }

    if (endDate) {
      const end = new Date(endDate).getTime();
      filtered = filtered.filter(b => b.timestamp.toDate().getTime() <= end);
    }
  ...
  return (
    <div className="space-y-8">
      ...
      <Modal isOpen={modalIsOpen} onRequestClose={closeModal} contentLabel="3D Viewer">
        <h2 className="text-xl font-semibold">3D Viewer for {selectedBenchmark?.proteinName}</h2>
        {selectedBenchmark && (
